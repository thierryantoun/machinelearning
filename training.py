...
F_targets_training = jax.vmap(advection_solver)(u0s_training)
...
F_targets_validation = jax.vmap(advection_solver)(u0s_validation)
...
for epoch in range(nb_epoch):
    for i in range(n_batches):
        ...
        params, opt_state = train_step(params, opt_state, u0s_batch, F_batch, lambda_consistency=10.0)

    if epoch % 10 == 0:
        loss_train = loss_fn(params, u0s_training, F_targets_training, lambda_consistency=10.0)
        loss_val   = loss_fn(params, u0s_validation, F_targets_validation, lambda_consistency=10.0)
